...
# Custom CSS - Fixed typo: "tyle" -> "style"
st.markdown("""
    <style>
    .main-header {
    font-size: 2.5rem;
...
if mode == "Single Candidate":
            ...
            with st.spinner("Analyzing candidate with RAG + LLM..."):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        # Gauge chart for score - Fixed spelling: "guage" -> "gauge"
                        ...
                        # Relevant Experience - Fixed syntax errors
...
elif mode == "Compare Candidate":  # Fixed: Changed "elif" indentation
    st.subheader("👥 Compare Multiple Candidates")  # Fixed: "Candidate" -> "Candidates"
    ...
    uploaded_files = st.file_uploader(  # Fixed: changed variable name to match later usage
        "Upload Multiple Resumes",
        type=["pdf", "docx", "txt"],  # Fixed: changed {} to []
        accept_multiple_files=True,
        help="Upload up to 10 resumes for comparison"
    )
    
    if st.button("🔍 Compare Candidates", type="primary"):  # Fixed: "Candidate" -> "Candidates"
        if job_description and uploaded_files:
            with st.spinner(f"Comparing {len(uploaded_files)} candidates..."):  # Fixed: uploaded_files
                try:
                    files = []
                    ...
                    if response.status_code == 200:
                        ...
                        # Show top candidate - Fixed syntax errors
                        st.success(f"🏆 Top Candidate: {result['top_candidate']}")
                        
                        # Create comparison dataframe
                        df = pd.DataFrame(result["candidates"])  # Fixed: "candidate" -> "candidates"
                        ...
                        with col2:
                            ...
                            fig_pie = px.pie(
                                ...
                                color_discrete_sequence=px.colors.qualitative.Set3  # Fixed: "color_discreate_sequence"
                            )
                        ...
                        for i, candidate in enumerate(result["candidates"]):  # Fixed: "candidate" -> "candidates"
                            with st.expander(f"#{i+1}: {candidate['filename']} - {candidate['score']}%"):  # Fixed syntax
                                st.write("**Strengths:**")
                                for strength in candidate["strengths"]:  # Fixed: "strength" -> "strengths"
                                    st.write(f"• {strength}")
                                st.write("**Weaknesses:**")
                                for weakness in candidate["weaknesses"]:
                                    st.write(f"• {weakness}")
                                st.write(f"**Recommendation:** {candidate['recommendation']}")  # Fixed: missing text
